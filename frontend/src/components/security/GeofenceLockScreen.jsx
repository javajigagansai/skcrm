...
import { ShieldAlert, Navigation, Lock, RefreshCw, Key, MapPin, CheckCircle2, AlertTriangle, ShieldCheck, Sparkles } from 'lucide-react';
...
export const GeofenceLockScreen = () => {
  ...
  const handleBypassSubmit = async (e) => {
    ...
    try {
      const res = await bypassGeofence(passcode);
      if (res.success) {
        setBypassSuccess(res.message);
        setTimeout(() => {
      ...
      setBypassError('Authorization error: ' + err.message);
    } finally {
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-8 shadow-2xl text-center space-y-6 animate-fadeIn relative overflow-hidden">
        ...
        <div className="space-y-2">
          <span className="badge bg-rose-500/20 text-rose-400 border border-rose-500/40 text-[10px] font-black uppercase tracking-wider px-3 py-1">
            🔒 GPS Geofence Security Perimeter Enforced
          </span>
          ...
          <p className="text-xs text-slate-400 leading-relaxed max-w-md mx-auto">
            {gpsStatus === 'CHECKING'
              ...
              : gpsStatus === 'OUTSIDE_FENCE'
              ? `You are currently outside the authorized ${(radius * 3.28084).toFixed(0)}ft perimeter for ${geofenceConfig.officeName || 'Office HQ'}.`
          ...
          <div className="bg-slate-950/80 rounded-2xl border border-slate-800 p-4 text-left text-xs space-y-3">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <span className="text-[11px] font-extrabold uppercase text-slate-400 flex items-center space-x-1.5">
                ...
                <span>Authorized Office Coordinates</span>
              </span>
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <span className="text-[11px] font-extrabold uppercase text-slate-400 flex items-center space-x-1.5">
                ...
                <span>Your Device GPS Location</span>
              </span>
            ...
            {distanceFromOffice !== null && (
              <div className="flex items-center justify-between pt-1">
                <span className="text-[11px] font-extrabold uppercase text-slate-400">Calculated Distance:</span>
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-slate-500 font-normal">
                    Allowed Radius: {(radius * 3.28084).toFixed(0)} Feet ({radius}m)
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="space-y-2.5 pt-2">
          ...
          {(geofenceConfig.allowAdminBypass || user?.role === 'SUPER_ADMIN' || user?.role === 'ADMIN' || user?.role === 'MANAGER') && (
            <button
              onClick={() => {
                setPasscode('');
                setBypassError('');
            ...
            >
              ...
              <span>Enter Emergency OTP Passcode</span>
            </button>
          )}
        </div>
            ...
            <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-sm w-full p-6 text-left space-y-4 shadow-2xl">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-white">Emergency Single-Use OTP</h3>
                    <p className="text-[10px] text-slate-400">Contact Admin for the rotating 6-digit passcode</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleBypassSubmit} className="space-y-3 text-xs">
                <p className="text-slate-400">
                  Enter the active 6-digit Emergency OTP. <strong className="text-amber-300">Note:</strong> Once used, this passcode will automatically expire and rotate to a new code.
                </p>
                <div>
                  <input
                    type="password"
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-white font-mono text-center text-sm tracking-widest font-black outline-none focus:border-amber-400"
                    autoFocus
                    maxLength={20}
                  />
                  {bypassError && (
                    <p className="text-[11px] text-rose-400 font-bold mt-2 flex items-center space-x-1">
                ...
                <div className="flex justify-end space-x-2 pt-2 border-t border-slate-800">
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-slate-800 text-slate-300 font-bold text-xs hover:bg-slate-700 transition cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs shadow transition cursor-pointer flex items-center space-x-1.5 disabled:opacity-50"
                  >
                    <Key className="h-3.5 w-3.5" />
                    <span>{isAuthorizing ? 'Verifying OTP...' : 'Authorize Session'}</span>
                  </button>
                </div>
              </form>
            </div>
